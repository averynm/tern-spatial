...
gps <- readRDS(file.path(outputbasepath, "data_working", "tern_gps_data_no_tripID.rds"))
...
#filter dataset
gps500 <- gps %>% dplyr::filter(colonyDist <= 5000)
#plot
brx <- seq(0, max(gps500$colonyDist), by = 100)

ggplot() +
  geom_histogram(data = gps500,
                 aes(colonyDist, fill = Species),
                 color = "black", breaks = brx) +
  scale_x_continuous(breaks = round(seq(0, max(gps500$colonyDist), by = 100), 0)) +
  scale_fill_viridis_d(begin = 0.1, end = 0.7) +
...
# b)use lead & lag of 'colony' column to identify trip start and end fixes:
...
for(i in 1:length(dlist)){
  ...
  png(filename = file.path(outputbasepath, "figs", "trip-splitting-plots",
                           paste0(dep$species,"-tag",
                                  dep$tag_id, "-birdID_", 
                                  dep$BirdID, ".png")),
