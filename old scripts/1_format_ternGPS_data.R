...
#2. tag data
#load data
#alltracks.raw <- read.csv(path = "tern_tracks_ALL_datetime.csv")
...
gpsdata <- alltracks.raw %>%
  ...
  na_if(9999.999) %>%
  mutate(lat = ifelse(is.na(Accuracy), NA, lat),
         lon = ifelse(is.na(Accuracy), NA, lon),
         ...
         BirdID = id,
         N_sat = qi,
         deployID = paste0("band", metalBand, "-tag", tagID, "-", yearDeploy)) %>%
  select(-Year, -Day, -Month, -Hour, -Minute, -Second, -Altitude, -Clock.Offset, -qi,
         -timestamp)

#remove missed fixes OR any with <5 satellites OR accuracy > 0.00003
gpsdata <- gpsdata %>% dplyr::filter(!is.na(lat) | N_sat < 5 | Accuracy > 0.00003)
...
rm(gpsdata)
#save dataframe
saveRDS(gps, "data_clean/arte_gps_data_no_tripID.rds")
