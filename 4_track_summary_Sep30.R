###############################################################################
#Tern gps data - summary info 
# based on script from Ariel Lenske
# Avery Nagy-MacArthur Sep 30 2024
#2024-09-30
###############################################################################
...
###############################################################################
#1. read in data
gps <- readRDS("data_clean/tern_gps_data_with_tripID.rds")
head(as.data.frame(gps))

#2. summary info

#num of birds
nrow(gps %>% select(metalBand) %>% distinct()) #51

#num of tracks
nrow(gps %>% select(deployID, tripID) %>% distinct()) #547

### num of complete/incomplete trips by island, species and year
knitr::kable(gps %>% 
               filter(!is.na(lat)) %>%
               select(deployID, location, species, tripID, yearDeploy, completeTrack) %>%
               distinct() %>%
